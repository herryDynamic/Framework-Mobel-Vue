...
<template>
  <div>
    ...
    <!-- 推送按钮 -->
    <div>
      <van-button type="info" class="save" @click="pushResumeTips">推送</van-button>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      pushResumeSucceed() {
        ...
        for (let index = 0; index < this.canSelectList.length; index++) {
          ...
          //list中清空被拒绝的简历
          // this.list = this.list.filter(t => t != this.canSelectList[index]);
        }
      ...
      // 推送简历-刘梦娇-2020-1-19 17:59:38
      pushResume() {
        var vm = this;
        this.recruitId = localStorage.getItem('recruitId')
        pushResume(this.recruitId, this.resumeId).then(response => {});
      }
    }
  };
</script>
